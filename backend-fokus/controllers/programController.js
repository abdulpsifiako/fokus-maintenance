...
module.exports = {
  ...
  getCountLatihanSoal: async (req, res) => {
    try {
      const { program_utama } = req.body;
      const [result, count] = await latihanModel
        .createQueryBuilder("latihan")
        .where("latihan.properties->>'program_utama' = :program_utama", {
          program_utama: program_utama,
        })
        .andWhere("latihan.is_deleted = false")
        .getManyAndCount();
      ...
      const totalSemuaSoal = result.reduce((total, item) => {
        const totalSoalPerLatihan = item.properties.latihan.reduce(
          (subTotal, latihanItem) => {
