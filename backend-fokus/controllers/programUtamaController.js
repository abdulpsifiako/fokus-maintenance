...
const programUtamaModel = conndb.getRepository(require("../entities/programUtamaEntity"))
const tryoutModel = conndb.getRepository(require("../entities/tryoutEntity"))
const kelasModel = conndb.getRepository(require("../entities/kelasOnlineEntity"))
...
module.exports={
    ...
    getProgramUtama:async (req, res) => {
        try {
            ...
            let query = programUtamaModel.createQueryBuilder().where("(LOWER(properties ->> 'name') LIKE LOWER(:name) OR properties ->> 'name' IS NULL)", { name: `%${search}%` }).andWhere('is_deleted = :isDeleted',{isDeleted:false}).andWhere("properties ->> 'status' = :status", { status: "true" });
            if (jenis) {
                query = query.andWhere("(LOWER(properties ->> 'jenis') LIKE LOWER(:jenis) OR properties ->> 'jenis' IS NULL)", { jenis: `%${jenis}%` })
            }else{
                ...
                query = query.andWhere("(properties ->> 'jenis' IN (:...nameList) OR properties ->> 'jenis' IS NULL)", { nameList })
            }
    ...
    getProgramUtamaAdmin:async (req, res) => {
        try {
            ...
            // let query = programUtamaModel.createQueryBuilder().where("(LOWER(properties ->> 'name') LIKE LOWER(:name) OR properties ->> 'name' IS NULL)", { name: `%${search}%` }).andWhere("(LOWER(properties ->> 'jenis') LIKE LOWER(:jenis) OR properties ->> 'jenis' IS NULL)", { jenis: `%${jenis}%` }).andWhere('is_deleted = :isDeleted',{isDeleted:false})
            let query = programUtamaModel.createQueryBuilder().where("(LOWER(properties ->> 'name') LIKE LOWER(:name) OR properties ->> 'name' IS NULL)", { name: `%${search}%` }).andWhere('is_deleted = :isDeleted',{isDeleted:false})
            if (jenis) {
                query = query.andWhere("(LOWER(properties ->> 'jenis') LIKE LOWER(:jenis) OR properties ->> 'jenis' IS NULL)", { jenis: `%${jenis}%` })
            }else{
                const jenis = await jenisProgramModel.find({
    ...
    updateProgramUtama:async (req, res) => {
        try {
            ...
            const programUtama = await programUtamaModel.findOneBy({id:id, is_deleted:false})
            ...
            await programUtamaModel.update({id:id},{
    ...
    getListProgramUtamaVideo:async (req, res) => {
        try {
            const result = await programUtamaModel
                ...
                .andWhere("program.properties ->> 'status' = :status ",{status :true})
    ...
    getProgramKu:async (req, res) => {
        try {
            ...
            const dataId = await transaksiModel.find({
                where:{
                        user_id:id,
                        jenis:jenis
                    },
            ...
            const ids = dataId.map(item => Number(item.program_id))
