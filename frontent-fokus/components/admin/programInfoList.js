...
export default function ProgramUtamaList({ onTambah, onEdit, onView }) {
  ...
  const toggleStatus = async (data) => {
    ...
    try {
      const response = await updateProgramUtama(
        data.id,
        {
          ...data,
          ...data,
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await getProgramUtamaAdmin(
        page,
        limit,
        search,
        null,
  ...
  const handleDeleteProgramUtama = async (id) => {
    try {
      const res = await deleteProgramUtama(id, Cookies.get("token"));
      if (res.status == 200) {
        setAlert({
          type: "success",
          title: "Info",
  ...
  return (
    <div className="space-y-4 font-poppins">
      <h1 className="text-lg font-semibold text-primary">Info Program Utama</h1>
        ...
        <table className="w-full text-sm">
          <thead className="bg-primary text-white text-left">
            <tr>
              ...
              <th className="p-2">Jenis Program</th>
              <th className="p-2">Total Pengajar</th>
            ...
            {dataProgram &&
              dataProgram.data.map((d, i) => (
                ...
                >
                  ...
                  <td className="p-2">{d.properties.name}</td>
                  <td className="p-2">{d.properties.jenis}</td>
                  <td className="p-2">
                    {d.properties?.pengajar.length} Pengajar
                  </td>
