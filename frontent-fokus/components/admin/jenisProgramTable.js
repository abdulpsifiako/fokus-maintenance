...
function JenisProgramTable({ onAdd, onEdit, onDetail }) {
  ...
  const toggleStatus = async (data) => {
    ...
    try {
      const response = await updateJenisProgram(
        data.id,
  ...
  const handleDeleteJenis = async (params) => {
    try {
      const response = await deleteJenisProgram(params, Cookies.get("token"));
      setAlert({
        type: "success",
        title: "Info",
        message: response.message || "Failed to fetch data",
...
export default JenisProgramTable;
