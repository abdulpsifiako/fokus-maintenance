...
function JenisKelasTable({ onAdd, onEdit, onDetail }) {
  ...
  const toggleStatus = async (data) => {
    ...
    try {
      const response = await updateJenisKelas(
        data.id,
  ...
  const handleDeleteJenis = async (params) => {
    try {
      const response = await deleteJenisKelas(params, Cookies.get("token"));
      setAlert({
        type: "success",
        title: "Info",
        message: response.message || "Failed to fetch data",
...
export default JenisKelasTable;
