...
import { generateSoalTemplate, parseSoalExcel } from "./soalExcelUtils";

export const ImportSoalModal = ({ onClose, onImport }) => {
  ...
  const handleConfirmImport = () => {
    ...
    onImport(result.valid); // append ke datasoal yang sudah ada
    onClose();
  };

  return (
    <div className="fixed inset-0 z-50 bg-gray-300/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="p-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="flex items-center justify-between bg-gray-50 border rounded-lg p-3">
            <div>
              ...
              <p className="text-xs text-gray-500">
                Unduh template Excel — mendukung jumlah opsi fleksibel
                (kosongkan opsi yang tidak dipakai).
              </p>
            </div>
            <button
              type="button"
              onClick={() => generateSoalTemplate()}
          ...
          {parsing && (
            <p className="text-sm text-gray-500 text-center">Membaca file...</p>
          )}
          ...
          {result && (
            <div className="space-y-3">
              <div className="flex items-center gap-2 bg-green-50 border border-green-200 text-green-700 text-sm p-3 rounded-md">
                ...
                <span>
                  <strong>{result.valid.length}</strong> soal berhasil dibaca
                  dan siap ditambahkan
                  {result.errors.length > 0 && (
                    <>
                      ...
                      baris dilewati (lihat detail di bawah)
                    </>
                  )}
                </span>
              </div>
              ...
              {/* Preview ringkas soal yang valid */}
                      ...
                      <p className="font-semibold text-gray-700">
                        ...
                        <span
                          dangerouslySetInnerHTML={{
                            __html: soal.pertanyaan.slice(0, 80),
                          }}
                        />
                        {soal.pertanyaan.length > 80 && "..."}
                      </p>
                      <p className="text-gray-400 mt-1">
                        {soal.opsi.length} opsi · Kunci:{" "}
                        <strong>{soal.kunci}</strong>
                      </p>
                    </div>
                  ))}
                </div>
        ...
        <div className="flex justify-end gap-3 p-5 border-t">
          ...
          >
            Tambahkan{" "}
            {result?.valid?.length ? `${result.valid.length} Soal` : ""}
          </button>
        </div>
      </div>
    </div>
